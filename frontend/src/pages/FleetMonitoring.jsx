...
const mapMarkers = [
  ...
  { id: 3, name: 'Réseau Est Goma', lat: -1.6585, lng: 29.223, status: 'warning', count: 18400 },
  ...
  { id: 6, name: 'Station Mbuji-Mayi', lat: -6.136, lng: 23.5898, status: 'critical', count: 8900 },
  { id: 7, name: 'Zone Kananga', lat: -5.8962, lng: 22.4166, status: 'normal', count: 6700 },
  { id: 8, name: 'Réseau Port Matadi', lat: -5.8167, lng: 13.45, status: 'warning', count: 9100 },
  { id: 9, name: 'Zone Minière Kolwezi', lat: -10.7148, lng: 25.4664, status: 'normal', count: 15800 },
  { id: 10, name: 'Hub Kikwit', lat: -5.041, lng: 18.8162, status: 'normal', count: 4100 },
];
...
export default function FleetMonitoring() {
  ...
  useEffect(() => {
    ...
    socket.on('geofence_alert', (alert) => {
      ...
      setLiveFeedEvents((prev) => [newEvent, ...prev].slice(0, 20));
  ...
  const filteredEvents = feedFilter === 'Géofence' ? liveFeedEvents.filter((e) => e.isGeofence) : liveFeedEvents;
          ...
          <div className="bg-transparent border border-slate-800 rounded-xl overflow-hidden flex flex-col h-full min-h-[460px] relative">
            ...
            <div className="relative flex-1 overflow-hidden flex items-center justify-center">
              <ComposableMap
                ...
                projectionConfig={{ scale: 1350, center: [23.5, -2.8] }}
                ...
                {mapMarkers.map((marker) => (
                  ...
                  >
                    <g transform="translate(-10, -10)">
                      <circle cx="10" cy="10" r="8" className="opacity-40 animate-ping fill-orange-500" />
                      <circle cx="10" cy="10" r="8" className="fill-transparent stroke-orange-500 stroke-2" />
                      <text x="10" y="12.5" textAnchor="middle" fill="#ffffff" fontSize="6.5" fontWeight="bold">
              ...
              {filteredEvents.slice(0, 5).map((evt) => (
                <motion.div
                  ...
                  className={`bg-transparent p-2.5 rounded-xl border flex items-center justify-between gap-2 hover:border-slate-700 transition-colors ${
                    evt.isGeofence ? 'border-red-900/50 bg-red-950/5' : 'border-slate-800'
                  }`}
                >
